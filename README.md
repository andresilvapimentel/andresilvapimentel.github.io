# André Silva Pimentel

<p align="center">
  <img src="images/Minha foto_IA.jpg" alt="André Silva Pimentel" width="250"/>
</p>

<p align="center">
  <b>Associate Professor | Researcher | Computational Chemist</b>
</p>

---

## About Me

Welcome to my personal academic website.

I am **Prof. Dr. André Silva Pimentel**, a researcher working at the interface of:

- Computational Chemistry
- Artificial Intelligence in Chemistry
- Molecular Modeling and Simulation
- Drug Discovery
- Bioinformatics
- Scientific Publishing

My research focuses on developing computational and data-driven approaches to understand molecular systems and accelerate scientific drug discovery.

---

## Academic Positions

- Associate Professor, [Pontifical Catholic University of Rio de Janeiro]
- Researcher, [Department of Chemistry]
- Guest Editor, *Discover Chemistry* - Collection of "Advances in Digital Chemical Discovery"
- Coordinator of Research Projects in Computational Chemistry and Artificial Intelligence

---

## Awards and Recognition

- Cientista do Nosso Estado FAPERJ
- Pesquisador de Produtividade em Pesquisa 2 do CNPq
- Member of INCT Oncologia Translacional e Terapia Gênica (INCT OncoTTGen)
  
---

## Research Interests

### Computational Chemistry
- Molecular Dynamics Simulations
- Enhanced Sampling
- Molecular Modeling
- Drug Design
- Digital Chemical Discovery

### Artificial Intelligence
- Machine Learning for Chemical Data
- Deep Learning
- Explainable AI
- Generative AI for Molecular Design

### Bio(Chem)informatics applied to:
- Small bioactive molecules
- Protein Structure Prediction
- Peptide Therapeutics
- RNA Therapeutics

### Interest on Health Issues
- Cancer (glioblastoma multiforme, Lung Cancer, and breast cancer)
- Neurodevelopmental disorder (Attention deficit hyperactivity disorder (ADHD))
- Neurodegenerative Diseases (Alzheimer)

---

## Current Research Topics

- AI-assisted Drug Discovery
- Explainable Machine Learning Models
- Molecular Dynamics of Amyloid Aggregation for Alzheimer's Disease
- Peptide Classification Using Deep Learning
- Prediction of adverse drug effects
- Computational Toxicology (Mutagenicity, Endocrine Disruption, Neurotoxicity, Caspase Toxicity)
- Classification Model of Blood Brain Barrier
- Structural Alerts
- Prediction models of pIC50
- Multi Drug Resistance ABC E-Flux Pumps
- Telomerase Inhibitors

---

## Publications

Selected publications:

Rosa et al. Toxic Alerts of Endocrine Disruption Revealed by Explainable Artificial Intelligence. Environment & Health, v. 3, p. 321-333, 2025.

Rosa et al. Applying local interpretable model-agnostic explanations to identify substructures that are responsible for mutagenicity of chemical compounds. Molecular Systems Design & Engineering, v. 9, p. 920-936, 2024.

Gralha et al. Gotcha GPT: Ensuring the Integrity in Academic Writing. Journal of Chemical Information and Modeling, v. 64, p. 8091-8097, 2024.

Rosa et al. Identifying Substructures That Facilitate Compounds to Penetrate the Blood-Brain Barrier via Passive Transport Using Machine Learning Explainer Models. ACS Chemical Neuroscience, v. 15, p. 2144-2159, 2024.

Nascimento et al. Do Large Language Models Understand Chemistry? A Conversation with ChatGPT. Journal of Chemical Information and Modeling, v. 63, p. 1649-1655, 2023.

Nascimento et al. Generating structural alerts from toxicology datasets using the local interpretable model-agnostic explanations method. Digital Discovery, v. 2, p. 1311-1325, 2023.


A complete list of publications is available through:

| CV Lattes | http://lattes.cnpq.br/3172784582436163 |

---

## Academic Profiles

| Platform | Link |
|-----------|-----------|
| CV Lattes | http://lattes.cnpq.br/3172784582436163 |
| ORCID | https://orcid.org/0000-0002-1301-0561 |
| Google Scholar | https://scholar.google.com/citations?user=1V6N3uMAAAAJ&hl=en |
| ResearchGate | https://www.researchgate.net/profile/Andre-Pimentel-2 |
| LinkedIn | https://www.linkedin.com/in/andre-pimentel-39457a306/ |
| Instagram | https://www.instagram.com/profapimentel/ | 
| GitHub | https://github.com/andresilvapimentel |

---

## Research Interests

### Artificial Intelligence in Chemistry
Research projects focused on machine learning and deep learning applications in chemical sciences.

### Molecular Simulation
Computational studies involving molecular dynamics and biomolecular systems.

### Open Science
Development of open-source tools, datasets, and educational resources.

---

## Teaching

Courses and mentoring activities in:

- Physical Chemistry
- Computational Chemistry
- Artificial Intelligence
- Scientific Programming (Python)
- Drug Design

---

## Contact

📧 Email: a_pimentel@puc-rio.br

🌐 Website:
https://andresilvapimentel.github.io

📍 Location:
Rio de Janeiro, Brazil

---

## Selected Skills

### Programming

- Python
- PyTorch
- Tensorflow
- Kerass
- Scikit-learn
- SHAP
- LIME
- Anchor

### Computational Tools

- GROMACS
- AMBER
- Gaussian
- AutoDock
- VMD

---

## Repository Statistics

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=andresilvapimentel&show_icons=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=andresilvapimentel&layout=compact)

---

## Quote

> "Science advances through curiosity, rigor, and collaboration."
> 
> "Is there any conflict between science and religion? There is no conflict in the mind of God, but often there is conflict in the minds of men. (by Henry Eyring)"

---

© André Silva Pimentel
